...
def createPkg():
  ...
  if request.method =='POST':
    ...
    query = "INSERT INTO themes (name, address) SELECT name, address FROM (SELECT '%s' AS name,'%s' AS address) t WHERE NOT EXISTS (SELECT * FROM themes WHERE name = '%s')" % (themeName, themeAddress, themeName)
    cur.execute(query)
    db.commit()
